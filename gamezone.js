...
function epicPrice(g){
  ...
  if(t.fmtPrice&&t.fmtPrice.discountPrice&&t.fmtPrice.discountPrice!=="0")return t.fmtPrice.discountPrice;
  return"\u20A9 "+t.discountPrice.toLocaleString("ko-KR");
}
function epicOrigPrice(g){
  if(!g.price||!g.price.totalPrice)return"";
  var t=g.price.totalPrice;
  if(t.fmtPrice&&t.fmtPrice.originalPrice&&t.fmtPrice.originalPrice!=="0")return t.fmtPrice.originalPrice;
...
function renderSteamTop20(){
  ...
  for(var i=0;i<show;i++){
    ...
    else if(g.price_kr_formatted)pt=g.price_kr_formatted;
  ...
  if(btn&&top20Data.length>10){
    ...
    btn.onclick=function(){steamTopExpanded=!steamTopExpanded;renderSteamTop20();};
...
function renderDeals(){
  ...
  for(var i=0;i<Math.min(8,deals.length);i++){
    ...
    if(origP)html+='<span class="deal-orig">'+origP+'</span>';
    html+='<span class="deal-sale">'+saleP+'</span>';
    html+='</div></div></div>';
  }
  grid.innerHTML=html;
...
function buildEpicFreeCard(g,type){
  ...
  h+='<span class="deal-badge '+cls+'">'+badge+'</span>';
  if(orig&&orig!=="0")h+='<span class="deal-orig">'+orig+'</span>';
...
function renderEpicTop20(){
  ...
  if(btn&&epicAllGames.length>10){
    ...
    btn.onclick=function(){epicTopExpanded=!epicTopExpanded;renderEpicTop20();};
  }else if(btn){btn.style.display="none";}
}
...
function renderEpicDeals(){
  ...
  for(var i=0;i<epicDeals.length;i++){
    ...
    if(orig)html+='<span class="deal-orig">'+orig+'</span>';
    html+='<span class="deal-sale">'+sale+'</span>';
    html+='</div></div></a></div>';
  }
  grid.innerHTML=html;
  if(btn)btn.style.display="none";
...
function openModal(appid){
  ...
  if(g.is_free||g.price==="0"){priceHtml='<span class="modal-price-free">\uBB34\uB8CC</span>';}
    ...
    var disc=parseInt(g.discount,10);
    if(disc>0){priceHtml='<span class="deal-badge">-'+disc+'%</span> <span class="modal-price-sale">'+(g.price_kr_formatted||formatKRW(g.price))+'</span>';}
    else{priceHtml='<span class="modal-price-normal">'+(g.price_kr_formatted||formatKRW(g.price))+'</span>';}
  }
  var genreHtml="";
...
/* ===== ADMIN (checkPw is global for onclick) ===== */
function checkPw(){
  var input=$("pwInput");
  if(!input)return;
...
function renderSidebar(){
  ...
  for(var j=0;j<GAME_SIDES.length;j++){
    var gs=GAME_SIDES[j];
    html+='<div class="side-section side-game">';
  ...
  for(var i=0;i<GUIDE_LABELS.length;i++){
    (function(idx){
      ...
      fetch(url).then(function(r){return r.json();}).then(function(data){
        ...
        if(!data.feed||!data.feed.entry||!data.feed.entry.length){el.innerHTML='<span class="side-empty">\uAC8C\uC2DC\uAE00 \uC5C6\uC74C</span>';return;}
        var h="";
        ...
        el.innerHTML=h;
      }).catch(function(){var el=$("guide_"+idx);if(el)el.innerHTML='<span class="side-empty">\uBD88\uB7EC\uC624\uAE30 \uC2E4\uD328</span>';});
    })(i);
  }
}
  ...
  for(var j=0;j<GAME_SIDES.length;j++){
    (function(idx){
      ...
      fetch(url).then(function(r){return r.json();}).then(function(data){
        ...
        el.innerHTML=h;
      }).catch(function(){var el=$("sideGame_"+idx);if(el)el.innerHTML='<span class="side-empty">\uBD88\uB7EC\uC624\uAE30 \uC2E4\uD328</span>';});
    })(j);
  }
}
